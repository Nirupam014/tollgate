...
7. **Recommends cheaper models (dynamic, requirement-driven)** — for each node it derives the requirements from the workflow and searches the **whole catalog** for the cheapest model that still supports the node, then re-prices it (a cost lever; same token volume). Requirements come from: a built-in **intent classifier** (a deterministic lexical-embedding classifier over the node's prompt/id that infers the task domain — code, reasoning, classification, image, … — so a code task keeps a code-strong model and a classifier can drop to a tiny one, and an image/audio/embedding node is never swapped to a text model), the context window the node's predicted p95 tokens need, max-output, tool calling, and your provider allowlist. It does not rely on a hand-wired swap list; curated capability scores, when present, only refine the reported confidence. The classifier is deterministic and makes no model calls — it informs recommendations only and never touches the gate or fingerprint. Quality beyond the tier floor can't be verified without evaluation, so the swap stays advisory.
